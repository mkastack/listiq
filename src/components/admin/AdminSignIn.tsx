...
export function AdminSignIn({ onSignIn }: AdminSignInProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (showMFA) {
      if (mfaCode === "123456") { // Simulation of MFA verification
        onSignIn();
      } else {
      ...
      <main className="w-full md:w-1/2 lg:w-[45%] flex flex-col justify-center items-center p-8 md:p-12 lg:p-24 bg-white relative">
        <div className="w-full max-w-[400px]">
          ...
          <div className="flex items-center gap-3 mb-12 md:hidden">
            <div className="flex items-center bg-[#0F172A] px-4 py-2 rounded-lg shadow-xl shadow-slate-900/10">
              <span className="font-heading font-extrabold text-white tracking-tight text-xl">List</span>
              <span className="font-heading font-extrabold text-[#2563EB] tracking-tight text-xl">IQ</span>
            </div>
            <span className="bg-[#F43F5E] text-white text-[10px] font-bold px-2 py-1 rounded tracking-wider uppercase">ADMIN PANEL</span>
          </div>
          ...
          <div className="mb-10">
            ...
            <h1 className="text-4xl font-bold tracking-tight mb-2 text-[#0F172A] font-heading">Welcome Back</h1>
            <p className="text-[#64748B] text-lg">Please enter your details to access the control center.</p>
          </div>
            ...
            {!showMFA ? (
              <>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-[#0F172A]" htmlFor="email">Email address</label>
                  <div className="relative">
                    <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#64748B] text-[20px]">mail</span>
                    <input
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-sm font-semibold text-[#0F172A]" htmlFor="password">Password</label>
                    <a className="text-sm text-[#2563EB] hover:underline font-semibold" href="#">Forgot password?</a>
                  </div>
                  <div className="relative">
                    <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#64748B] text-[20px]">lock</span>
                    <input
                      className="w-full pl-10 pr-4 py-3.5 bg-white border border-[#E2E8F0] rounded-xl focus:ring-2 focus:ring-[#2563EB]/10 focus:border-[#2563EB] transition-all outline-none text-[#0F172A]"
                      id="password"
                      placeholder="••••••••"
                ...
                <div className="flex items-center">
                  <input 
                    ...
                    type="checkbox" 
                  />
                  <label className="ml-2 text-sm text-[#64748B]" htmlFor="remember">Remember me for 30 days</label>
                </div>
              </>
            ) : (
              <div className="space-y-4 animate-in fade-in zoom-in duration-300">
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6">
                  <p className="text-xs text-blue-700 font-medium leading-relaxed">
                    <span className="font-bold">Two-Step Verification:</span> We've sent a 6-digit code to your registered device. Enter it below to continue.
                  </p>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-[#0F172A]" htmlFor="mfa">Security Code</label>
                  <div className="relative">
                    <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#64748B] text-[20px]">verified_user</span>
                    <input
          ...
          <div className="mt-12 pt-8 border-t border-slate-100 flex flex-col gap-5">
            <div className="flex items-center gap-3 text-[#64748B]">
              <span className="material-symbols-outlined text-[22px] text-green-500" style={{ fontVariationSettings: "'FILL' 1" }}>shield_with_heart</span>
              <p className="text-sm font-medium">Secure, encrypted connection</p>
            </div>
            <p className="text-sm text-[#64748B]">
              Not an administrator? <Link className="text-[#2563EB] font-bold hover:underline" to="/">Return to home</Link>
            </p>
          </div>
        </div>
        ...
        <footer className="absolute bottom-8 left-12 right-12 hidden md:flex justify-between items-center text-[11px] font-medium text-slate-400">
          ...
          <div className="flex gap-4">
            <a className="hover:text-[#2563EB] transition-colors" href="#">Privacy</a>
            <a className="hover:text-[#2563EB] transition-colors" href="#">Terms</a>
            <a className="hover:text-[#2563EB] transition-colors" href="#">Security</a>
          </div>
        </footer>
      </main>
      ...
      <aside className="hidden md:flex md:w-1/2 lg:w-[55%] bg-[#0F172A] relative overflow-hidden items-center justify-center p-12">
        ...
        <div className="relative z-10 flex flex-col items-center text-center max-w-lg">
          <div className="flex items-center bg-white/5 backdrop-blur-xl border border-white/10 px-8 py-4 rounded-2xl mb-12 shadow-2xl">
            <span className="font-heading font-extrabold text-white tracking-tighter text-5xl">List</span>
            <span className="font-heading font-extrabold text-[#2563EB] tracking-tighter text-5xl">IQ</span>
          </div>
          ...
          <div className="mt-16 bg-slate-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2rem] flex items-center gap-6 shadow-2xl ring-1 ring-white/5">
            <div className="h-14 w-14 rounded-full bg-[#2563EB]/20 flex items-center justify-center">
              <span className="material-symbols-outlined text-[#2563EB] text-[32px]">trending_up</span>
            </div>
