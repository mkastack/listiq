...
export function SearchHero() {
    ...
    const fetchCitySuggestions = async () => {
      try {
        const response = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(city)}&featuretype=city&limit=5`,
        );
        if (response.ok) {
          const data = await response.json();
          // Filter to make sure they are somewhat unique and relevant
          const uniqueCities = Array.from(
            new Set(data.map((item: any) => item.display_name.split(",")[0].trim())),
          ).map((name) => {
  ...
  return (
    <div className="relative w-full">
      ...
      {showSuggestions &&
        ((activeField === "q" && suggestions.length > 0) ||
                ...
                {activeField === "q" &&
                  suggestions.map((item, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-foreground group-hover:text-primary">
                            {item.type === "category" ? item.name : item.name}
                          </p>
                          {item.city && (
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                        Select →
                      </span>
                    </Link>
                  ))}
