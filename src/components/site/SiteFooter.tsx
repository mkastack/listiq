        ...
        <div className="grid gap-12 md:grid-cols-5">
          <div className="md:col-span-2">
            ...
            <p className="mt-4 max-w-sm text-sm text-brand-foreground/70">
              Find it. List it. Own it. The smarter way to discover the businesses that move Ghana
              forward.
            </p>
            <div className="mt-6 flex items-center gap-3">
              {[Facebook, Instagram, Twitter, Linkedin].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="grid h-9 w-9 place-items-center rounded-full bg-white/5 text-brand-foreground/80 transition-colors hover:bg-white/10 hover:text-brand-foreground"
                >
...
function FooterCol({ title, links }: { title: string; links: { to: string; label: string }[] }) {
  return (
    <div>
      <h4 className="font-display text-sm font-semibold uppercase tracking-wider text-brand-foreground/90">
        {title}
      </h4>
