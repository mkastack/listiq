...
export function Sidebar({ isOpen = false, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  const [profile, setProfile] = useState<{ full_name: string; avatar_url: string; subscription_plan: string } | null>(null);
    ...
    <>
      ...
      <div className="px-4 mb-6 flex items-center gap-3">
        ...
        <div className="flex flex-col">
          <span className="text-sm font-semibold text-white truncate max-w-[140px]">{rawName}</span>
          <span className="text-[11px] font-medium text-slate-500">{profile?.subscription_plan || 'Free Plan'}</span>
        </div>
      </div>

      <nav className="flex-1 space-y-1 overflow-y-auto no-scrollbar" style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
        <style dangerouslySetInnerHTML={{ __html: `.no-scrollbar::-webkit-scrollbar { display: none; }` }} />
        <Link
          to="/dashboard"
          activeOptions={{ exact: true }}
          className="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all duration-200"
          activeProps={{ className: "text-white bg-blue-600/10 border-l-2 border-blue-600 font-semibold" }}
          inactiveProps={{ className: "text-slate-400 font-medium hover:bg-slate-800/50 hover:text-white border-l-2 border-transparent" }}
        >
