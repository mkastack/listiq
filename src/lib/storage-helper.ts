    ...
    if (uploadError.message.includes('bucket not found') || (uploadError as any).status === 404) {
      ...
      try {
        ...
        if (!createError) {
          ...
          throw new Error(`Bucket "${bucket}" was not found and could not be created automatically. Please create it manually in your Supabase dashboard under Storage -> New Bucket.`);
        }
      } catch (err) {
        throw new Error(`STORAGE ERROR: The bucket "${bucket}" does not exist. \n\nFIX: Go to your Supabase Dashboard -> Storage -> Create a NEW public bucket named "${bucket}".`);
      }
    }
    throw uploadError;
  }
