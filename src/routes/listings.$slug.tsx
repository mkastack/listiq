...
import { Phone, Globe, MessageCircle, MapPin, BadgeCheck, Mail, Clock, Share2, Bookmark, Flag } from "lucide-react";
import { SiteShell } from "@/components/site/SiteShell";
import { ListingCard, type ListingCardData } from "@/components/site/ListingCard";
import { StarRating } from "@/components/site/StarRating";
import { fetchListingBySlug, fetchReviews, fetchSimilarListings, addReview, type ListingRow } from "@/lib/queries";
  ...
  notFoundComponent: () => (
    <SiteShell>
      <div className="mx-auto max-w-2xl px-4 py-32 text-center">
        ...
        <p className="mt-3 text-muted-foreground">This listing may have been removed or never existed.</p>
        <Link to="/search" className="mt-6 inline-flex rounded-xl bg-foreground px-5 py-2.5 text-sm font-semibold text-background">Browse all listings</Link>
      </div>
    </SiteShell>
  ),
});
...
function ListingDetailPage() {
      ...
      try {
        ...
        if (l) {
          ...
          const [r, s, { data: savedData }] = await Promise.all([
            ...
            session?.user ? supabase.from('saved_listings').select('*').eq('user_id', session.user.id).eq('listing_id', l.id).maybeSingle() : Promise.resolve({ data: null })
          ]);
          ...
          // REAL-TIME REVIEWS
          channel = supabase.channel(`listing-reviews-${l.id}`)
            .on('postgres_changes', { 
              event: '*', 
              schema: 'public', 
              table: 'reviews',
  ...
  const handleSave = async () => {
    ...
    if (isSaved) {
      await supabase.from('saved_listings').delete().eq('user_id', session.user.id).eq('listing_id', listing.id);
      setIsSaved(false);
    } else {
      await supabase.from('saved_listings').insert({ user_id: session.user.id, listing_id: listing.id });
      setIsSaved(true);
    }
  };
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      await addReview(listing.id, session.user.id, newReview.rating, newReview.title, newReview.body);
    ...
    <SiteShell>
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="-mt-12 md:-mt-16 grid gap-8 lg:grid-cols-[1fr_340px] relative z-10">
          {/* Main */}
          <div>
            <div className="rounded-3xl border border-border bg-card p-6 md:pt-10 md:px-8 md:pb-8" style={{ boxShadow: "var(--shadow-card)" }}>
              <p className="text-xs font-semibold uppercase tracking-widest text-primary">{listing.category.replace(/-/g, " ")}</p>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                ...
                {listing.city && (
                  <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4" /> {listing.city}, {listing.region}</span>
                )}
              </div>

              <div className="mt-6 flex flex-wrap gap-2">
                {listing.phone && <ActionButton href={`tel:${listing.phone}`} icon={Phone}>Call</ActionButton>}
                {listing.whatsapp && <ActionButton href={`https://wa.me/${listing.whatsapp.replace(/[^0-9]/g, "")}`} icon={MessageCircle}>WhatsApp</ActionButton>}
                {listing.website && <ActionButton href={listing.website} icon={Globe}>Website</ActionButton>}
                <button 
                  onClick={handleSave}
            ...
            <div className="mt-8 border-b border-border">
              <div className="flex gap-1">
                {TABS.map((t) => (
                  ...
                  >
                    {t} {t === "Reviews" && reviews.length > 0 && <span className="ml-1 text-xs text-muted-foreground">({reviews.length})</span>}
                  </button>
                ))}
              </div>
            </div>
                  ...
                  {listing.keywords && listing.keywords.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {listing.keywords.map((k) => (
                        <span key={k} className="rounded-full bg-accent px-3 py-1 text-xs text-foreground">{k}</span>
                      ))}
                    </div>
                  )}
                  <div className="grid gap-4 sm:grid-cols-2">
                    {listing.address && <InfoRow icon={MapPin} label="Address" value={`${listing.address}, ${listing.city || ""}`} />}
                    {listing.phone && <InfoRow icon={Phone} label="Phone" value={listing.phone} />}
                    {listing.email && <InfoRow icon={Mail} label="Email" value={listing.email} />}
                    {listing.website && <InfoRow icon={Globe} label="Website" value={listing.website} />}
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-muted/30 p-6 rounded-2xl border border-border">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">See what others are saying about this business.</p>
                    </div>
                    <button 
                  ...
                  {reviews.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {reviews.map((r) => (
                        <div key={r.id} className="rounded-2xl border border-border bg-card p-6 shadow-sm hover:border-primary/20 transition-all">
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                              <StarRating value={r.rating} showCount={false} size="md" />
                              <span className="text-xs font-bold text-muted-foreground">•</span>
                              <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{new Date(r.created_at).toLocaleDateString()}</span>
                            </div>
                          </div>
                          {r.title && <h4 className="font-bold text-foreground text-lg mb-1">{r.title}</h4>}
          ...
          <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-border bg-card p-6" style={{ boxShadow: "var(--shadow-card)" }}>
              <h3 className="font-display text-base font-semibold text-foreground">Quick contact</h3>
              <div className="mt-4 space-y-3 text-sm">
                {listing.phone && <SidebarRow icon={Phone} label={listing.phone} href={`tel:${listing.phone}`} />}
                {listing.email && <SidebarRow icon={Mail} label={listing.email} href={`mailto:${listing.email}`} />}
                {listing.website && <SidebarRow icon={Globe} label={listing.website.replace(/^https?:\/\//, "")} href={listing.website} />}
                {listing.address && <SidebarRow icon={MapPin} label={`${listing.address}, ${listing.city || ""}`} />}
              </div>
            </div>
        ...
        <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden flex flex-col border border-slate-100 animate-in zoom-in-95 duration-300">
          <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-gradient-to-r from-primary/5 to-transparent">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Share your experience with {listing?.name}</p>
            </div>
            <button onClick={() => setShowReviewModal(false)} className="w-10 h-10 rounded-full hover:bg-slate-100 transition-colors flex items-center justify-center text-slate-400">
          ...
          <form onSubmit={handleSubmitReview} className="p-8 space-y-6">
            <div className="flex flex-col items-center gap-4 py-4 bg-slate-50 rounded-2xl border border-slate-100">
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Rate your experience</p>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((s) => (
                  ...
                  >
                    <Star className={`h-8 w-8 ${s <= newReview.rating ? "fill-warning text-warning" : "text-slate-300"}`} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Headline</label>
              <input
                required
            ...
            <div className="space-y-2">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Your Feedback</label>
              <textarea
                required
...
function ActionButton({ href, icon: Icon, children }: { href: string; icon: any; children: React.ReactNode }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-primary-foreground" style={{ background: "var(--gradient-primary)" }}>
...
function SidebarRow({ icon: Icon, label, href }: { icon: any; label: string; href?: string }) {
  ...
  return href ? <a href={href} className="block hover:text-primary">{inner}</a> : inner;
}