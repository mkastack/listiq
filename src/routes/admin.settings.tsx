    ...
    <main className="max-w-[1440px] mx-auto px-6 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 text-slate-400 mb-2">
          ...
          <span className="text-[10px] text-blue-600 font-black uppercase tracking-widest">Configurations</span>
        </div>
        <h1 className="text-2xl font-bold text-slate-900">Platform Settings</h1>
        <p className="text-sm text-slate-500 font-medium">Manage API credentials, security protocols, and third-party integrations.</p>
      </div>
        ...
        {["Integrations", "SEO", "Notifications", "Security"].map((tab) => (
          <button
            ...
            className={`pb-4 text-[11px] font-black uppercase tracking-widest transition-all ${
              activeTab === tab ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-400 hover:text-slate-900'
            }`}
          >
            {tab}
          </button>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-10">
            ...
            <>
              ...
              <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-lg font-bold text-slate-900 leading-none mb-1.5">Payment Gateway</h2>
                      <p className="text-xs text-slate-500 font-medium">Configure Paystack for subscription billing and claims processing.</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Secret Authorization Key</label>
                    <input
                  ...
                  <div className="space-y-2.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Public Access Key</label>
                    <input
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-mono text-sm focus:ring-4 focus:ring-blue-600/5 focus:border-blue-600 focus:bg-white outline-none transition-all"
                      type="text"
                ...
                <div className="mt-10 flex justify-end gap-3">
                  <button disabled={isSaving} onClick={handleSave} className="px-8 py-3.5 bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-blue-700 transition-all active:scale-95 shadow-xl shadow-blue-600/20 flex items-center gap-2">
              ...
              <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                <div className="mb-8">
                  ...
                  <p className="text-xs text-slate-500 font-medium">Configure API keys for AI-powered content generation and rewriting.</p>
                </div>
                <div className="space-y-2.5 max-w-xl">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">OPENAI API KEY (CHATGPT)</label>
              ...
              <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                <div className="mb-8">
                  ...
                  <p className="text-xs text-slate-500 font-medium">Configure map layers and location search parameters.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-8">
                    <div className="space-y-2.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">GOOGLE PLACES API KEY</label>
                      <input
          ...
          {activeTab === "SEO" && (
            <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
              ...
              <p className="text-sm text-slate-500 mb-8">Manage how your platform appears in search engines.</p>
              <div className="space-y-6">
                <div className="space-y-2.5">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Site Meta Title</label>
                  <input className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm" placeholder="ListIQ - Business Directory" />
                </div>
                <div className="space-y-2.5">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Meta Description</label>
                  <textarea className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm h-32" placeholder="The #1 business directory in Ghana..." />
                </div>
              </div>
            </section>
          )}
          ...
          {activeTab === "Notifications" && (
            <section className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
              ...
              <div className="p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">mNotify API AUTH KEY</label>
                    <input
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-mono text-sm focus:bg-white transition-all"
                      type="password"
                  ...
                  <div className="space-y-2.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">SENDER IDENTITY</label>
                    <input
                      ...
                      onChange={(e) => setSettings({...settings, mnotify_sender_id: e.target.value})}
                    />
                  </div>
                </div>
          ...
          {activeTab === "Security" && (
            <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
              <h2 className="text-lg font-bold text-slate-900 mb-4 text-rose-600">Platform Security</h2>
              <p className="text-sm text-slate-500 mb-8">Manage authentication protocols and system access.</p>
              <div className="space-y-6">
                <div className="flex items-center justify-between p-6 bg-rose-50 border border-rose-100 rounded-2xl">
                  <div>
                    <p className="text-sm font-bold text-rose-900 leading-none mb-1.5">Multi-Factor Authentication</p>
                    <p className="text-xs text-rose-600">Require MFA for all administrator accounts.</p>
                  </div>
                  <button 
                    onClick={() => {
                  ...
                  >
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all duration-300 ${settings.mfa_enabled ? 'left-7' : 'left-1'}`}></div>
                  </button>
                </div>
              </div>
              <div className="mt-10 flex justify-end">
                <button disabled={isSaving} onClick={handleSave} className="px-8 py-3.5 bg-rose-600 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-rose-700 transition-all active:scale-95 shadow-xl shadow-rose-600/20 flex items-center gap-2">
                  {isSaving && <span className="material-symbols-outlined text-sm animate-spin">sync</span>}
        ...
        <div className="col-span-12 lg:col-span-4 space-y-10">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
            ...
            <ul className="space-y-8">
              {[
                { title: "API Compliance", sub: "Guidelines for secure credential storage.", icon: "verified_user" },
                { title: "System Heartbeat", sub: "Last successful sync: 2 minutes ago.", icon: "monitor_heart" },
              ].map((item) => (
                <li key={item.title} className="flex items-start gap-4 group cursor-pointer">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-800 group-hover:text-blue-600 transition-colors leading-tight mb-1">{item.title}</p>
                    <p className="text-[11px] text-slate-400 leading-relaxed font-medium">{item.sub}</p>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="bg-white border-2 border-rose-100 rounded-3xl p-8 shadow-xl shadow-rose-600/5">
            <div className="flex items-center gap-3 mb-6 text-rose-600">
              ...
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em]">Security Protocol</h3>
            </div>
            <div className="space-y-6">
              <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                Resetting configuration will disconnect all live APIs and clear session tokens. This action is irreversible.
              </p>
              <div className="space-y-2.5">
                <label className="text-[9px] font-black text-rose-300 uppercase tracking-widest">VERIFICATION TOKEN</label>
                <input
