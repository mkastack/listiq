...
function AddListing() {
  ...
  const handleSubmit = async () => {
    ...
    if ((currentListings || 0) >= limit) {
      alert(
        `Limit Reached: Your ${currentPlan} plan only allows up to ${limit} listings. Please upgrade your plan in the Billing section to add more.`,
      );
      setIsSubmitting(false);
      return;
    }
            ...
            {[
              {
                id: 1,
                label: "Basic Info",
                status: step === 1 ? "Current Step" : step > 1 ? "Completed" : "Pending",
              },
              {
                id: 2,
                label: "Location",
                status: step === 2 ? "Current Step" : step > 2 ? "Completed" : "Pending",
              },
              {
                id: 3,
                label: "Media",
                status: step === 3 ? "Current Step" : step > 3 ? "Completed" : "Pending",
              },
              { id: 4, label: "Preview", status: step === 4 ? "Current Step" : "Pending" },
            ].map((s) => (
              <div
                key={s.id}
                className={`flex items-center gap-4 shrink-0 lg:shrink ${s.status === "Pending" ? "opacity-40" : ""}`}
              >
                <div
                ...
                <div className="hidden sm:block">
                  <p
                    className={`text-sm font-bold whitespace-nowrap ${s.status === "Current Step" ? "text-blue-600" : "text-slate-700"}`}
                  >
                    {s.label}
                  </p>
              ...
              <div className="bg-gradient-to-br from-blue-600/5 to-transparent border-2 border-dashed border-blue-600/20 rounded-2xl p-6 relative overflow-hidden group">
                <div className="flex flex-col md:flex-row items-start justify-between relative z-10 gap-6">
                  <div className="flex gap-4">
                    <div
                      className={`w-12 h-12 rounded-xl bg-blue-600/10 flex items-center justify-center text-blue-600 shrink-0 ${isGenerating ? "animate-pulse" : ""}`}
                    >
                      <span
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 max-w-md leading-relaxed">
                        Let our AI draft your listing description and select the best keywords based
                  ...
                  <div className="flex flex-col items-end gap-3 w-full md:w-auto">
                    <div className="bg-white/80 p-1 rounded-lg border border-slate-200 flex items-center">
                      <button className="px-3 py-1 text-xs font-bold bg-blue-600 text-white rounded-md shadow-sm">
                        GPT-4o
                      </button>
                    ...
                    >
                      <span
                        className={`material-symbols-outlined text-sm ${isGenerating ? "animate-spin" : ""}`}
                        style={{ fontVariationSettings: "'FILL' 1" }}
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap items-center gap-2 p-2 bg-white border border-slate-200 rounded-lg focus-within:border-blue-600 focus-within:ring-4 focus-within:ring-blue-600/10 transition-all min-h-[42px]">
                        {formData.keywords.map((tag) => (
                          <span
                            key={tag}
                            className="bg-blue-600/10 text-blue-600 px-2 py-1 rounded text-xs font-bold flex items-center gap-1"
                          >
                            {tag}{" "}
                            <span
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              ...
                              if (target.value) {
                                setFormData({
                                  ...formData,
                                  keywords: [...formData.keywords, target.value],
                                });
                                target.value = "";
                              }
                            }
                ...
                <div className="p-8 space-y-8">
                  <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-12 hover:border-blue-500 hover:bg-blue-50/50 transition-all cursor-pointer relative group">
                    ...
                    <p className="text-xs text-slate-500">
                      Click to upload or drag and drop. PNG or JPG (Max 2MB)
                    </p>
                    <input
                      type="file"
                ...
                <div className="pt-16 pb-10 px-10">
                  <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                    <div>
                      ...
                      <p className="text-blue-600 font-bold mt-1 uppercase tracking-widest text-xs">
