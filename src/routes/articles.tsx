...
export const Route = createFileRoute("/articles")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Stories, guides and insights about doing business in Ghana.",
      },
    ],
  }),
    ...
    <SiteShell>
      <section className="mx-auto max-w-7xl px-4 py-16 md:px-6">
        ...
        <h1 className="mt-2 font-display text-4xl font-extrabold tracking-tight md:text-5xl">
          Stories from the directory
        </h1>
        <p className="mt-3 max-w-xl text-muted-foreground">
          Guides, news and ideas for living, working and building in Ghana.
        </p>
              ...
              <div className="flex flex-1 flex-col gap-3 p-6">
                ...
                <div className="mt-auto flex flex-wrap gap-1.5 pt-2">
                  {(a.tags ?? []).slice(0, 3).map((t: string) => (
                    <span
                      key={t}
                      className="rounded-full bg-accent px-2.5 py-0.5 text-[10px] uppercase tracking-wider text-foreground"
                    >
                      {t}
                    </span>
                  ))}
