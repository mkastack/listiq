...
function AdminClaims() {
  ...
  const fetchClaims = async () => {
    ...
    const { data: claimsData, error } = await supabase
      .from("claim_requests")
      .select(
        "*, listings(name, city, logo_url), profiles:claimant_id(full_name, email, avatar_url), reviewer:reviewed_by(full_name, avatar_url)",
      )
    ...
    <main className="max-w-[1440px] mx-auto px-6 py-8">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium">
            Review and process business ownership claim requests
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={fetchClaims}
            className="bg-white border border-slate-200 px-4 py-2.5 rounded-xl text-xs font-bold text-slate-700 flex items-center gap-2 hover:bg-slate-50 transition-all shadow-sm uppercase tracking-widest"
          >
            <span
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[
          ...
          <div
            key={stat.label}
            className="bg-white border border-slate-200 p-6 rounded-2xl flex flex-col justify-between shadow-sm transition-all hover:shadow-md"
          >
            <div className="flex justify-between items-start">
              <div
                className={`p-2 bg-${stat.color}-50 text-${stat.color}-600 rounded-lg border border-${stat.color}-100`}
              >
                <span className="material-symbols-outlined">{stat.icon}</span>
              </div>
              <span
                className={`text-[9px] font-black text-${stat.color}-600 bg-${stat.color}-50 px-2.5 py-1 rounded-full uppercase tracking-[0.1em] border border-${stat.color}-100`}
              >
                {stat.trend}
              </span>
            </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {claims.map((claim) => (
                <tr key={claim.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    {claim.verification_document_url ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-blue-600 text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 hover:underline"
                      >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black tracking-widest uppercase border ${
                    ...
                    {claim.reviewer ? (
                      <div className="flex items-center gap-2">
                        <img
                          ...
                          src={claim.reviewer.avatar_url}
                          alt="Reviewer"
                        />
                        <span className="text-[11px] text-slate-700 font-bold">
                          {claim.reviewer.full_name.split(" ")[0]}
                        </span>
                      </div>
                    ) : (
                      <span className="text-slate-400 italic text-[10px] font-bold uppercase tracking-widest">
                        Unassigned
                      </span>
                    )}
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      {claim.status === "pending" && (
                        <>
                          <button
                            onClick={() =>
                              handleApprove(claim.id, claim.listing_id, claim.claimant_id)
                            }
              ...
              {claims.length === 0 && !isLoading && (
                <tr>
                  ...
                  >
                    No claim requests found in the database.
                  </td>
                </tr>
              )}
            </tbody>
