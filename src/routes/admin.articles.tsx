...
function AdminArticles() {
  ...
  const fetchContent = async () => {
    ...
    const [
      ...
      supabase.from('articles').select('*', { count: 'exact', head: true }).eq('is_ai_generated', true),
    ]);
    ...
    <main className="max-w-[1440px] mx-auto px-6 py-8">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <nav className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] text-blue-600 font-bold uppercase tracking-widest">Content Engine</span>
          </nav>
          <h1 className="text-2xl font-bold text-slate-900">Articles & News</h1>
          <p className="text-sm text-slate-500 font-medium">Moderate platform content and manage automated news feeds</p>
        </div>
        <div className="bg-white px-5 py-4 border border-slate-200 rounded-2xl flex items-center gap-8 shadow-sm">
          <div className="flex flex-col">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">PUBLISHED</span>
            <span className="text-xl font-bold text-slate-900">{isLoading ? '...' : stats.published.toLocaleString()}</span>
          </div>
          <div className="w-px h-8 bg-slate-100"></div>
          <div className="flex flex-col">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">DRAFTS</span>
            <span className="text-xl font-bold text-slate-900">{isLoading ? '...' : stats.drafts.toLocaleString()}</span>
          </div>
          <div className="w-px h-8 bg-slate-100"></div>
          <div className="flex flex-col">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">AI AGENT</span>
            <span className="text-xl font-bold text-blue-600">{isLoading ? '...' : stats.aiGenerated.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 mb-6 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={triggerFetch} className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">
          ...
          >
            {isGenerating ? (
              ...
              <span className="material-symbols-outlined text-[18px] group-hover:rotate-12 transition-transform">colors_spark</span>
            )}
      ...
      <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-6 mb-8 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            <span className="text-[9px] text-emerald-700 font-black uppercase tracking-[0.2em]">AUTOPILOT MODE</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="md:col-span-2">
            <label className="text-[9px] font-black text-emerald-800 mb-2 block uppercase tracking-[0.2em]">CONFIGURED FEEDS</label>
            <div className="flex flex-wrap gap-2 p-2 bg-white border border-emerald-100 rounded-xl min-h-[44px] shadow-sm">
              {fetchJobs.length > 0 ? fetchJobs.map((job) => (
                <span key={job.id} className="bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg text-[10px] flex items-center gap-2 border border-emerald-100 font-black uppercase tracking-wider">
                  {job.source_url.replace('https://', '')}
                  <span className="material-symbols-outlined text-[14px] cursor-pointer hover:text-rose-500 transition-colors">close</span>
                </span>
              )) : (
                <span className="text-emerald-300 text-[10px] p-2 font-bold uppercase">No feeds configured</span>
              )}
            </div>
          </div>
          <div>
            <label className="text-[9px] font-black text-emerald-800 mb-2 block uppercase tracking-[0.2em]">SCAN FREQUENCY</label>
          ...
          <div>
            <label className="text-[9px] font-black text-emerald-800 mb-2 block uppercase tracking-[0.2em]">AI REWRITE LEVEL</label>
            <select className="w-full bg-white border border-emerald-100 rounded-xl py-2.5 px-4 text-xs font-bold text-slate-600 outline-none focus:ring-4 focus:ring-emerald-500/10">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50 border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">CONTENT PREVIEW</th>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">SOURCE / AUTHOR</th>
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">ENGAGEMENT</th>
                <th className="px-6 py-4 text-right"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {articles.map((article) => (
                <tr key={article.id} className={`hover:bg-slate-50/50 transition-colors ${article.status === 'draft' ? 'border-l-4 border-l-amber-400' : ''}`}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      {article.image_url ? (
                        <img className="w-16 h-10 object-cover rounded-lg border border-slate-200 shadow-sm" src={article.image_url} alt="Article Cover" />
                      ) : (
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-slate-900 line-clamp-1 text-sm">{article.title}</span>
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{new Date(article.created_at).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black uppercase shadow-sm ${article.is_ai_generated ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-600'}`}>
                        {article.is_ai_generated ? 'AI' : 'US'}
                      </div>
                      <span className="text-xs font-bold text-slate-700">{article.is_ai_generated ? 'Auto Agent' : 'Platform Staff'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest border ${
                      article.is_ai_generated ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-slate-50 text-slate-600 border-slate-200'
                    }`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-[9px] font-black uppercase tracking-widest ${article.status === 'published' ? 'text-emerald-700' : 'text-amber-700'}`}>
                        {article.status}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="font-mono text-xs text-slate-700 font-black">{article.views_count?.toLocaleString() || 0}</span>
              ...
              {articles.length === 0 && !isLoading && (
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center text-slate-400 italic font-medium uppercase tracking-widest text-[10px]">No news entries found in database.</td>
                </tr>
              )}
            </tbody>
          </table>
