    ...
    <div className="p-4 md:p-6 lg:p-8 w-full pb-24 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-body-base">
            Manage and monitor all your business listings
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex bg-white rounded-lg border border-slate-200 p-1 shadow-sm">
            <button className="px-4 py-1.5 text-xs font-bold rounded-md bg-slate-100 text-slate-900">
              All
            </button>
            <button className="px-4 py-1.5 text-xs font-bold rounded-md text-slate-500 hover:text-slate-900 transition-colors">
              Active
            </button>
            <button className="px-4 py-1.5 text-xs font-bold rounded-md text-slate-500 hover:text-slate-900 transition-colors">
              Pending
          ...
          <table className="w-full text-left border-collapse min-w-full">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="px-4 py-4 text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                  Business
                </th>
                <th className="hidden md:table-cell px-4 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                  Category
                </th>
                <th className="px-4 py-4 text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-widest text-center">
                  Status
                </th>
                <th className="hidden lg:table-cell px-4 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest text-center">
                  Plan
                </th>
                <th className="hidden sm:table-cell px-4 py-4 text-[10px] md:text-[11px] font-bold text-slate-500 uppercase tracking-widest text-center">
                  Views
                </th>
                <th className="hidden md:table-cell px-4 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                  Created
              ...
              {listings.length > 0 ? (
                listings.map((listing) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-slate-100 flex-shrink-0 flex items-center justify-center overflow-hidden border border-slate-200">
                          {listing.logo_url ? (
                            <img src={listing.logo_url} className="w-full h-full object-cover" />
                          ) : (
                            <span className="material-symbols-outlined text-slate-400">store</span>
                          )}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <p className="font-bold text-slate-900 text-sm truncate max-w-[120px] md:max-w-[200px]">
                            {listing.name}
                          </p>
                    ...
                    <td
                      ...
                      ref={activeDropdown === listing.id ? dropdownRef : null}
                    >
                      <button
                        onClick={() =>
                ...
                <tr>
                  <td colSpan={9} className="px-6 py-12 text-center text-slate-500 italic">
                    {isLoading
                      ? "Loading listings..."
                      : "No listings found. Start by adding your first business!"}
                  </td>
                </tr>
              )}
            </tbody>
