...
function MyReviews() {
  ...
  const statCards = [
    { label: "Average Rating", value: stats.avgRating.toString(), icon: "star", color: "text-amber-400", bg: "bg-blue-50" },
    { label: "Total Reviews", value: stats.total.toString(), trend: "+0 this week", icon: "reviews", color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Response Rate", value: `${stats.responseRate}%`, icon: "bolt", color: "text-purple-600", bg: "bg-purple-50", progress: stats.responseRate },
  ];
    ...
    <div className="p-4 md:p-8 w-full max-w-7xl mx-auto pb-24 space-y-8 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-slate-500 font-body-base">Manage and respond to feedback across all your listings</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {statCards.map((s, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col min-h-[170px] group cursor-pointer hover:shadow-md transition-all duration-300">
            <div className="flex items-center justify-between mb-4">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">{s.label}</p>
              <div className={`w-12 h-12 rounded-full ${s.bg} flex items-center justify-center ${s.color} shrink-0 group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shadow-sm`}>
            ...
            <div className="flex items-baseline gap-3 mt-auto mb-4">
              <span className="text-3xl font-bold text-slate-900">{isLoading ? '...' : s.value}</span>
              {s.trend && <span className="text-emerald-600 text-xs font-bold shrink-0">{s.trend}</span>}
            </div>
            <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
              <div className={`${s.color.replace('text', 'bg')} h-full rounded-full transition-all duration-1000`} style={{ width: s.progress ? `${s.progress}%` : "0%" }}></div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-6">
        {reviews.length > 0 ? reviews.map((review) => (
          <div key={review.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-all">
            <div className="p-6 md:p-8">
              <div className="flex flex-col sm:flex-row items-start justify-between gap-4 mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-slate-900">{review.profiles?.full_name || 'Anonymous'}</h3>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">On:</span>
              ...
              {review.owner_response ? (
                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 relative mt-4">
                  ...
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">
                    Replied on {new Date(review.responded_at || review.created_at).toLocaleDateString()}
                  </span>
                </div>
              ) : (
          ...
          <div className="bg-white rounded-2xl p-12 text-center border border-slate-100 shadow-sm">
            ...
            <p className="text-slate-500 max-w-sm mx-auto">
              {isLoading ? 'Fetching your latest reviews...' : "You haven't received any reviews yet. Share your listings to start gathering feedback!"}
            </p>
          </div>
        )}
      </div>
