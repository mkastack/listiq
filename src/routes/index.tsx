...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Discover trusted businesses across Ghana and West Africa. Restaurants, hotels, lawyers, tech, and more — all on ListIQ." },
      { property: "og:title", content: "ListIQ — Find It. List It. Own It." },
      { property: "og:description", content: "Discover trusted businesses across Ghana and West Africa." },
    ],
  }),
  component: Home,
});
    ...
    <SiteShell>
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-24 pt-20 md:px-6 md:pb-32 md:pt-28">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="mt-6 font-display text-4xl font-extrabold leading-[1.05] tracking-tight md:text-6xl">
              Find every business that<br className="hidden md:block" />
              <span className="bg-gradient-to-r from-primary-glow to-primary bg-clip-text text-transparent"> matters near you.</span>
            </h1>
            <p className="mx-auto mt-5 max-w-xl text-base text-brand-foreground/70 md:text-lg">
              ListIQ is the modern directory for restaurants, hotels, lawyers, tech, beauty and more — verified, reviewed and ready to connect.
            </p>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 md:px-6">
        ...
        <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7">
          {categories.slice(0, 14).map((c) => (
            <CategoryCard key={c.id} slug={c.slug} name={c.name} icon={c.icon} count={c.listing_count} />
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 md:px-6">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {[
            { n: "01", icon: Search, title: "Search", body: "Type a keyword or category. Filter by city, rating, or what's open." },
            { n: "02", icon: Star, title: "Find", body: "Compare verified businesses with real reviews and photos." },
            { n: "03", icon: ArrowRight, title: "Connect", body: "Call, message on WhatsApp, or visit the website in one tap." },
          ].map((s) => (
            <div key={s.n} className="relative overflow-hidden rounded-3xl border border-border bg-card p-8" style={{ boxShadow: "var(--shadow-card)" }}>
              <span className="font-display text-5xl font-extrabold text-muted-foreground/20">{s.n}</span>
      ...
      <section className="bg-surface">
        <div className="mx-auto max-w-7xl px-4 py-20 md:px-6">
          <SectionHead eyebrow="Just listed" title="Fresh on ListIQ" link={{ to: "/search", label: "See all" }} />
      ...
      {/* CTA banner */}
      <section className="mx-auto max-w-7xl px-4 py-20 md:px-6">
        <div className="relative overflow-hidden rounded-3xl bg-brand p-10 text-brand-foreground md:p-16" style={{ boxShadow: "var(--shadow-elevated)" }}>
          ...
          <div className="relative grid gap-8 md:grid-cols-2 md:items-center">
            <div>
              ...
              <p className="mt-4 max-w-md text-brand-foreground/70">
                {session ? "You're logged in! Go to your dashboard to manage your listings and track your growth." : "Get found by thousands of customers searching every day. Free forever to start. Upgrade for premium placement."}
              </p>
            </div>
            <div className="flex flex-col gap-3 md:items-end">
              {session ? (
                <Link to="/dashboard" className="inline-flex items-center justify-center gap-2 rounded-xl px-6 py-3 font-display text-sm font-semibold text-primary-foreground" style={{ background: "var(--gradient-primary)", boxShadow: "var(--shadow-glow)" }}>
                  Go to Dashboard <ArrowRight className="h-4 w-4" />
                </Link>
              ) : (
                <Link to="/auth" search={{ mode: "signup" }} className="inline-flex items-center justify-center gap-2 rounded-xl px-6 py-3 font-display text-sm font-semibold text-primary-foreground" style={{ background: "var(--gradient-primary)", boxShadow: "var(--shadow-glow)" }}>
                  Create your free listing <ArrowRight className="h-4 w-4" />
                </Link>
              )}
              <Link to="/pricing" className="text-sm text-brand-foreground/70 hover:text-brand-foreground">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-20 md:px-6">
        <SectionHead eyebrow="From the blog" title="Stories, guides and insights" link={{ to: "/articles", label: "All articles" }} />
          ...
          {articles.map((a) => (
            ...
            >
              ...
              <div className="flex flex-1 flex-col gap-3 p-6">
                ...
                <h3 className="font-display text-lg font-semibold leading-tight text-foreground group-hover:text-primary">{a.title}</h3>
...
function Stat({ label, value, icon: Icon }: { label: string; value: number | string; icon: any }) {
  return (
    <div className="flex flex-col items-center gap-1 rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur">
      ...
      <span className="text-[10px] font-bold uppercase tracking-widest text-brand-foreground/50">{label}</span>
    </div>
  );
}

function SectionHead({ eyebrow, title, subtitle, link }: { eyebrow: string; title: string; subtitle?: string; link?: { to: string; label: string } }) {
  return (
    <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
      ...
      {link && (
        <Link to={link.to} className="flex items-center gap-2 text-sm font-bold text-foreground transition-colors hover:text-primary">
