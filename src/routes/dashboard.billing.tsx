...
function BillingPlan() {
  ...
  const fetchBillingData = async () => {
    ...
    // Fetch Usage Stats
    const [{ count: listingsUsed }, { data: listingsData }, { count: aiUsed }] = await Promise.all([
      supabase
        .from("listings")
    ...
    const imagesCount =
      listingsData?.reduce((acc, curr) => acc + (curr.gallery_images?.length || 0), 0) || 0;
    ...
    if (profile) {
      ...
      setUserPlan({
        ...
        renewal_date: profile.subscription_renewal
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      ...
      const profileSub = supabase
        .channel(`billing-profile-${session.user.id}`)
        .on(
          "postgres_changes",
          {
            event: "UPDATE",
      ...
      const paymentsSub = supabase
        .channel(`billing-payments-${session.user.id}`)
        .on(
          "postgres_changes",
          {
            event: "INSERT",
  ...
  const handlePayment = async (planName: string, amount: number) => {
    ...
    const { data: settings } = await supabase
      ...
      .maybeSingle();
    const paystackKey =
      settings?.paystack_public_key || "pk_test_62a882da2b5348732a1220f494288036431bc140";
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8">
              ...
              <div className="md:text-right">
                <p className="text-4xl font-bold font-display leading-tight">
                  GHS {userPlan.price}
                </p>
          ...
          <div className="space-y-8 flex-1">
            {[
              ...
              },
              {
                ...
                used: usage.ai.used,
                total: usage.ai.total,
                color: "bg-amber-500",
                warning: usage.ai.used / usage.ai.total > 0.8,
              },
            ].map((meter) => (
              <div key={meter.label}>
          ...
          {plans.map((p) => (
            <div
              ...
              className={`bg-white border-2 rounded-2xl p-6 flex flex-col transition-all group hover:shadow-lg ${
                p.active
                  ? "border-blue-600 shadow-xl shadow-blue-600/10 scale-105 relative z-10"
            ...
            >
              ...
              <div className="flex items-baseline gap-1 mt-3">
                <p
                  className={`text-3xl font-bold ${p.active ? "text-blue-600" : "text-slate-900"}`}
                >
              ...
              <ul className="mt-8 space-y-4 flex-1">
                {p.features.map((f) => (
                  ...
                  >
                    <span
                      className={`material-symbols-outlined text-[18px] ${p.active ? "text-blue-600" : "text-slate-400"}`}
                    >
                      check
                    </span>
                    {f}
                ...
                history.map((h) => (
                  <tr key={h.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-block px-2.5 py-1 rounded-full text-[10px] font-bold ${h.status === "Paid" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                      >
                        {h.status}
                      </span>
                    </td>
