...
function AdminOverview() {
  ...
  const fetchStats = async () => {
    ...
    const [
      ...
      supabase.from('listings').select('*', { count: 'exact', head: true }).gte('created_at', prevStart.toISOString()).lt('created_at', prevFilter.toISOString()),
      supabase.from('profiles').select('*', { count: 'exact', head: true }),
      supabase.from('profiles').select('*', { count: 'exact', head: true }).gte('created_at', prevStart.toISOString()).lt('created_at', prevFilter.toISOString()),
      supabase.from('listings').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
      supabase.from('claim_requests').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
      supabase.from('payments').select('amount').gte('created_at', currentFilter.toISOString()).eq('status', 'success'),
      supabase.from('payments').select('amount').gte('created_at', prevStart.toISOString()).lt('created_at', prevFilter.toISOString()).eq('status', 'success'),
      supabase.from('sms_logs').select('*', { count: 'exact', head: true }).gte('created_at', todayStart.toISOString())
    ]);
  ...
  const fetchRecentData = async () => {
    const [listingsRes, reviewsRes] = await Promise.all([
      supabase.from('listings').select('id, name, created_at').order('created_at', { ascending: false }).limit(3),
      supabase.from('reviews').select('id, rating, created_at, listings(name)').order('created_at', { ascending: false }).limit(2)
    ]);
    ...
    const combinedActivities = [
      ...(listingsRes.data || []).map(l => ({
        ...
        icon: 'post_add',
        color: 'blue',
        text: <><span className="font-bold">New Listing:</span> {l.name}</>,
        time: l.created_at
      })),
      ...(reviewsRes.data || []).map(r => ({
        ...
        icon: 'star',
        color: 'amber',
        text: <><span className="font-bold">New Review:</span> {r.rating} stars on {(r.listings as any)?.name}</>,
        time: r.created_at
      }))
  ...
  useEffect(() => {
    ...
    const listingsSub = supabase.channel('admin-listings-overview')
    ...
    const profilesSub = supabase.channel('admin-profiles-overview')
    ...
    const claimsSub = supabase.channel('admin-claims-overview')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'claim_requests' }, () => {
        fetchStats();
      })
      .subscribe();

    const reviewsSub = supabase.channel('admin-reviews-overview')
  ...
  return (
    <div className="animate-in fade-in duration-500">
          ...
          {["Last 24h", "7 Days", "30 Days", "All Time"].map((range) => (
            <button
              ...
              className={`px-4 py-1.5 text-[11px] font-bold uppercase tracking-wider transition-all rounded ${
                timeRange === range ? 'bg-[#2563EB] text-white shadow-sm' : 'text-[#64748B] hover:bg-slate-50'
              }`}
            >
              {range}
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6 mb-8">
        <StatCard title="Total Listings" value={stats.totalListings.toLocaleString()} trend={stats.trends.listings} />
        <StatCard title="Total Users" value={stats.totalUsers.toLocaleString()} trend={stats.trends.users} />
        <StatCard title="Revenue" value={`$${stats.revenue.toLocaleString()}`} trend={stats.trends.revenue} />
        <StatCard 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
        <div className="lg:col-span-2 bg-white border border-[#E2E8F0] p-6 rounded-xl">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="material-symbols-outlined text-[#64748B] cursor-pointer">more_horiz</span>
          </div>
          <div className="h-64 relative bg-slate-50 flex items-end justify-between px-2 pb-8 rounded overflow-hidden">
            <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
              <path d="M0,80 Q10,75 20,60 T40,45 T60,55 T80,30 T100,20" fill="none" stroke="#2563EB" strokeWidth="2"></path>
              <path d="M0,80 Q10,75 20,60 T40,45 T60,55 T80,30 T100,20 L100,100 L0,100 Z" fill="url(#grad)" opacity="0.1"></path>
              <defs>
            ...
            {['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'].map((day, i) => (
              <div key={day} className="flex flex-col items-center z-10">
                <div className="w-1 bg-slate-200 rounded-full opacity-50" style={{ height: [32, 24, 40, 28, 44, 36, 52][i] + 'px' }}></div>
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-40 h-40 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle cx="80" cy="80" fill="transparent" r="70" stroke="#2563EB" strokeDasharray="440" strokeDashoffset="110" strokeWidth="12"></circle>
                <circle cx="80" cy="80" fill="transparent" r="70" stroke="#F43F5E" strokeDasharray="440" strokeDashoffset="380" strokeWidth="12"></circle>
              </svg>
              <div className="absolute text-center">
                <span className="block font-h2 text-h2">{stats.totalListings > 1000 ? (Math.round(stats.totalListings / 100) / 10) + 'k' : stats.totalListings}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-[#E2E8F0] p-6 rounded-xl">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="material-symbols-outlined text-[#64748B] cursor-pointer">filter_list</span>
          </div>
          <div className="divide-y divide-slate-100">
            {recentSignups.map((profile) => (
              <SignupItem 
                ...
                tierColor={profile.plan === 'enterprise' ? 'bg-[#2563EB]' : profile.plan === 'pro' ? 'bg-amber-500' : 'bg-slate-200 !text-slate-700'}
...
function StatCard({ title, value, trend, action, footer, footerIcon, borderClass = "" }: any) {
  return (
    <div className={`bg-white border border-[#E2E8F0] p-5 rounded-lg ${borderClass}`}>
      ...
      {action && (
        <Link to={action.to} className="text-[#2563EB] font-body-sm font-semibold hover:underline mt-1 flex items-center">
          {action.label} <span className="material-symbols-outlined text-sm ml-1">chevron_right</span>
        </Link>
      )}
      {footer && (
        <div className="flex items-center text-[#64748B] font-body-sm mt-1">
          {footerIcon && <span className="material-symbols-outlined text-sm mr-1">{footerIcon}</span>}
          <span>{footer}</span>
        </div>
      )}
    </div>
  ...
  return (
    <div className="p-3 bg-slate-50 rounded border border-[#E2E8F0] flex items-center justify-between">
      <div className="flex items-center gap-3">
        <span className={`w-2 h-2 rounded-full ${color === 'status-green' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
        <div>
          <p className="font-label-caps text-label-caps text-[#0F172A]">{label}</p>
          <p className="text-[10px] text-[#64748B]">{detail}</p>
        </div>
      </div>
      <span className={`font-mono text-xs font-bold ${color === 'status-green' ? 'text-emerald-600' : 'text-amber-600'}`}>{status}</span>
    </div>
  );
}
...
function ActivityItem({ icon, color, text, time }: any) {
  ...
  return (
    <div className="relative pl-10">
      <span className={`absolute left-0 w-6 h-6 rounded-full border-2 flex items-center justify-center z-10 ${colors[color]}`}>
...
function SignupItem({ name, email, tier, tierColor, avatar, initials }: any) {
  return (
    <div className="py-4 flex items-center justify-between">
      <div className="flex items-center gap-3">
        {avatar ? (
          <img src={avatar} alt={name} className="w-10 h-10 rounded-full object-cover border border-slate-100" />
        ) : (
      ...
      <span className={`px-2 py-1 ${tierColor} text-white text-[10px] font-bold rounded uppercase tracking-wider`}>
        {tier}
      </span>
    </div>
  );
